...
function viewMission(id) {
  ...
  render(
    h(`div.fade-in.${meta.cls}`,
      ...
      h('article.dossier.dossier--brief',
        h('header.dossier__head',
          h('span.dossier__kind', t('doc.assignment')),
          h('span.dossier__no', t(`tier.${raw.tier}`)),
        ),
        isDone ? h('div.dossier__stamp', t('doc.filed')) : null,
        h('h2.dossier__title', m.title),
        h('p.dossier__lede', m.brief),
      ),
